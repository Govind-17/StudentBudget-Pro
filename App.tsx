...
const App: React.FC = () => {
  ...
  const availableMonths = useMemo(() => {
    ...
    const maxDateForInterval = endOfMonth(new Date());
  ...
  const filterSummary = useMemo(() => {
    ...
    if (filterMonth !== 'all') {
      ...
      if (isValid(parsedMonth)) activeFilters.push(format(parsedMonth, 'MMM yyyy'));
    }
  ...
  const handleShareHistory = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = image;
      link.download = `History_${format(new Date(), 'MMM_dd_yyyy')}.png`;
      link.click();
    } catch (err) { console.error(err); } finally { setIsSharing(false); }
  };
    ...
    <div className={`min-h-screen pb-24 relative ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-gray-50 text-slate-900'}`}>
                ...
                <div key={t.id} className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-slate-800">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">{isValid(d) ? format(d, 'MMM dd') : 'Invalid'}</p>
                    </div>
                  </div>
      ...
      <header className={`sticky top-0 z-30 px-6 py-4 flex items-center justify-between border-b ${isDarkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-gray-200'} backdrop-blur-md`}>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center"><Wallet className="w-5 h-5 text-white" /></div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8">
              ...
              <div className="flex items-end justify-between mt-1">
                ...
                <div className="text-right">
                  <div className="w-32 h-1.5 bg-white/20 rounded-full mt-1 overflow-hidden">
                    <div className="h-full bg-white rounded-full transition-all" style={{ width: `${Math.min((stats.totalExpense / monthlyBudget) * 100, 100)}%` }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={`p-6 rounded-2xl ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">Expense Breakdown</h3>
              {pieChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={pieChartData} cx="50%" cy="50%" outerRadius={80} fill="#8884d8" dataKey="value" labelLine={false}>
                      {pieChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                    </Pie>
                    <Tooltip formatter={(value: any, name: any) => [`₹${Number(value).toFixed(0)}`, name]} />
                  </PieChart>
                </ResponsiveContainer>
              ) : <div className="text-center py-6 text-gray-400 italic">No data this month.</div>}
            </div>

            <div className={`p-6 rounded-2xl ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">6-Month Spending</h3>
              {barChartData.some(d => d.expenses > 0) ? (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={barChartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <YAxis tickFormatter={(v) => `₹${v.toFixed(0)}`} tick={{ fill: isDarkMode ? '#cbd5e1' : '#64748b', fontSize: 10 }} />
                    <Tooltip formatter={(v: any) => `₹${Number(v).toFixed(0)}`} />
                    <Bar dataKey="expenses" fill="#6366f1" />
                  </BarChart>
                </ResponsiveContainer>
              ) : <div className="text-center py-6 text-gray-400 italic">No spending trends yet.</div>}
            </div>

            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold flex items-center gap-2"><PiggyBank className="w-5 h-5 text-indigo-500" /> Savings Goals</h3>
                <button onClick={() => { setEditingGoalId(null); setGoalTitle(''); setGoalTarget(''); setGoalCurrent('0'); setIsGoalModalOpen(true); }} className="p-1.5 rounded-full bg-indigo-100 text-indigo-600 active:scale-90 transition-all"><Plus className="w-4 h-4" /></button>
              </div>
                    ...
                    return (
                      <div key={goal.id} className={`p-4 rounded-2xl ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
                        <div className="flex justify-between items-start mb-2">
                          <div><p className="font-bold text-sm">{goal.title}</p><p className="text-xs text-gray-500">₹{goal.currentAmount.toFixed(0)} / ₹{goal.targetAmount.toFixed(0)}</p></div>
                          <button onClick={() => setGoalToDelete(goal.id)} className="p-1.5 text-rose-600"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                        <div className="w-full h-4 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden relative">
                          <div className="h-full transition-all flex items-center justify-end pr-2" style={{ width: `${progress}%`, backgroundColor: goal.color }}>
        ...
        {activeTab === 'history' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setIsFilterVisible(!isFilterVisible)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${isFilterVisible ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 dark:bg-slate-800 text-gray-600'}`}>
                <Filter className="w-3.5 h-3.5" /> Filters
              </button>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-2">
              <div className={`p-4 rounded-2xl flex items-center gap-3 ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white shadow-sm'}`}>
                <div className="w-9 h-9 bg-emerald-100 dark:bg-emerald-500/10 rounded-xl flex items-center justify-center"><TrendingUp className="w-4 h-4 text-emerald-600" /></div>
                <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Income</p><p className="font-bold text-emerald-600">₹{filteredStats.income.toFixed(0)}</p></div>
              </div>
              <div className={`p-4 rounded-2xl flex items-center gap-3 ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white shadow-sm'}`}>
                <div className="w-9 h-9 bg-rose-100 dark:bg-rose-500/10 rounded-xl flex items-center justify-center"><TrendingDown className="w-4 h-4 text-rose-600" /></div>
                <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Expense</p><p className="font-bold text-rose-600">₹{filteredStats.expense.toFixed(0)}</p></div>
              </div>
            </div>
            ...
            {isFilterVisible && (
              <div className={`p-4 rounded-2xl border mb-4 space-y-4 scale-in ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-10 pr-4 py-2 rounded-xl text-sm border focus:ring-2 focus:ring-indigo-500 outline-none ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50 border-gray-200'}`} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Month</label>
                    <select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className={`w-full px-3 py-2 rounded-xl text-xs border appearance-none ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`}>
                      <option value="all">All Months</option>
                      {availableMonths.map(m => <option key={m.toISOString()} value={m.toISOString()}>{format(m, 'MMMM yyyy')}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Category</label>
                    <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className={`w-full px-3 py-2 rounded-xl text-xs border appearance-none ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`}>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="flex gap-1">
                    {(['all', 'income', 'expense'] as const).map(t => (
                      <button key={t} onClick={() => setFilterType(t)} className={`flex-1 py-2 rounded-xl text-[10px] font-bold capitalize ${filterType === t ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 dark:bg-slate-800'}`}>{t}</button>
                    ))}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <input type="date" value={filterStartDate} onChange={(e) => setFilterStartDate(e.target.value)} className={`w-full px-3 py-2 rounded-xl text-xs border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} />
                  <input type="date" value={filterEndDate} onChange={(e) => setFilterEndDate(e.target.value)} className={`w-full px-3 py-2 rounded-xl text-xs border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} />
                </div>
                <button onClick={handleClearFilters} className="w-full py-2 bg-gray-100 dark:bg-slate-800 text-gray-600 rounded-xl text-sm font-semibold flex items-center justify-center gap-2"><XCircle className="w-4 h-4" /> Clear</button>
              </div>
            )}
                  ...
                  <div key={t.id} onClick={() => handleOpenDetail(t)} className={`p-4 rounded-2xl flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all hover:translate-x-1 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{isValid(d) ? format(d, 'MMM dd') : '...'}</p>
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            <div className={`p-6 rounded-2xl ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold flex items-center gap-2"><Grid className="w-5 h-5 text-indigo-500" /> Categories</h3>
                <button onClick={() => setIsCategoryModalOpen(true)} className="p-1.5 rounded-full bg-indigo-100 text-indigo-600"><Plus className="w-4 h-4" /></button>
              </div>
              <div className="space-y-2">
                {categories.filter(c => c.id !== 'cat-income').map(cat => (
                  <div key={cat.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-slate-800">
                    ...
                    {!cat.isSystem && <button onClick={() => setCategoryToDelete(cat.id)} className="p-2 text-gray-400 active:text-rose-600"><Trash2 className="w-3.5 h-3.5" /></button>}
                  </div>
                ))}
              </div>
            </div>
      ...
      <nav className={`fixed bottom-0 left-0 right-0 z-50 px-6 py-3 border-t backdrop-blur-lg flex justify-between items-center ${isDarkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-white/80 border-gray-200'}`}>
        <button onClick={() => setActiveTab('dashboard')} className={`flex flex-col items-center gap-1 ${activeTab === 'dashboard' ? 'text-indigo-600' : 'text-gray-400'}`}><LayoutDashboard className="w-6 h-6" /><span className="text-[10px] font-bold">Home</span></button>
        <button onClick={() => setActiveTab('history')} className={`flex flex-col items-center gap-1 ${activeTab === 'history' ? 'text-indigo-600' : 'text-gray-400'}`}><History className="w-6 h-6" /><span className="text-[10px] font-bold">History</span></button>
        <button onClick={() => setActiveTab('settings')} className={`flex flex-col items-center gap-1 ${activeTab === 'settings' ? 'text-indigo-600' : 'text-gray-400'}`}><Settings className="w-6 h-6" /><span className="text-[10px] font-bold">Settings</span></button>
      </nav>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm overflow-y-auto">
          <div className={`w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 transition-all ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            <form onSubmit={handleAddTransaction} className="space-y-4">
              <div className="grid grid-cols-2 gap-2 p-1 bg-gray-100 dark:bg-slate-800 rounded-xl">
                <button type="button" onClick={() => setType('expense')} className={`py-2 rounded-lg text-sm font-semibold ${type === 'expense' ? 'bg-white dark:bg-slate-700 shadow' : 'text-gray-500'}`}>Expense</button>
                <button type="button" onClick={() => { setType('income'); setTransactionCategory('cat-income'); }} className={`py-2 rounded-lg text-sm font-semibold ${type === 'income' ? 'bg-white dark:bg-slate-700 shadow' : 'text-gray-500'}`}>Income</button>
              </div>
              <input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} required />
              {type === 'expense' && (
                <select value={transactionCategory} onChange={(e) => setTransactionCategory(e.target.value)} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`}>
                  {categories.filter(c => c.id !== 'cat-income').map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
                </select>
              )}
              <input type="text" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} />
              <div className="space-y-1">
                <input type="file" ref={fileInputRef} onChange={handleImageUpload} accept="image/*" className="hidden" />
                {!receiptImage ? (
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full p-4 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-2 text-gray-400 hover:text-indigo-500 transition-all"><Camera className="w-6 h-6" /><span className="text-xs">Attach Receipt</span></button>
                ) : (
                  <div className="relative"><img src={receiptImage} alt="Receipt" className="w-full h-32 object-cover rounded-2xl" /><button type="button" onClick={() => setReceiptImage(undefined)} className="absolute top-2 right-2 p-1.5 bg-rose-500 text-white rounded-full"><X className="w-4 h-4" /></button></div>
                )}
              </div>
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg"><Save className="w-5 h-5" /> Save</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isDetailModalOpen && viewingTransaction && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm overflow-y-auto">
          <div className={`w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 transition-all scale-in ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold">Details</h2><button onClick={() => setIsDetailModalOpen(false)}><X className="w-5 h-5" /></button></div>
            <div className="space-y-6">
              <div className="text-center py-6">
                <span className={`text-4xl font-black ${viewingTransaction.type === 'income' ? 'text-emerald-500' : 'text-rose-500'}`}>{viewingTransaction.type === 'income' ? '+' : '-'}₹{viewingTransaction.amount.toFixed(2)}</span>
                <p className="text-sm font-bold uppercase text-gray-400 mt-2">{viewingTransaction.type}</p>
              </div>
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-slate-800/50 flex items-center gap-4">
                  <div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-600"><Tag className="w-5 h-5" /></div>
                  <div><p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Category</p><p className="font-semibold text-sm">{getCategoryById(viewingTransaction.category).name}</p></div>
                </div>
                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-slate-800/50 flex items-center gap-4">
                  <div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-600"><FileText className="w-5 h-5" /></div>
                  <div><p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Description</p><p className="font-semibold text-sm">{viewingTransaction.description}</p></div>
                </div>
                {viewingTransaction.receiptImage && <img src={viewingTransaction.receiptImage} className="w-full rounded-2xl" alt="Receipt" />}
              </div>
              <div className="flex gap-3 pt-4">
                <button onClick={() => { handleOpenEdit(viewingTransaction); setIsDetailModalOpen(false); }} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-bold active:scale-95 transition-all">Edit</button>
                <button onClick={() => { setTransactionToDelete(viewingTransaction.id); setIsDetailModalOpen(false); }} className="flex-1 py-4 bg-gray-100 dark:bg-slate-800 text-rose-500 rounded-2xl font-bold active:scale-95 transition-all">Delete</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {isSaveConfirmOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
          <div className={`w-full max-w-xs p-6 rounded-3xl text-center shadow-2xl scale-in ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
             ...
             <p className="text-sm text-gray-500 mb-6">Confirm and update your budget record.</p>
             <div className="flex flex-col gap-2">
               <button onClick={handleFinalizeSave} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-lg">Confirm</button>
               <button onClick={() => setIsSaveConfirmOpen(false)} className="w-full py-3 bg-gray-100 dark:bg-slate-800 rounded-xl font-semibold">Cancel</button>
             </div>
          </div>
        </div>
      )}
      ...
      {isGoalModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className={`w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 transition-all ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold">New Savings Goal</h2><button onClick={() => setIsGoalModalOpen(false)}><X className="w-5 h-5" /></button></div>
            <form onSubmit={(e) => { e.preventDefault(); if(!goalTitle || !goalTarget) return; const newGoal: SavingsGoal = { id: crypto.randomUUID(), title: goalTitle, targetAmount: parseFloat(goalTarget), currentAmount: parseFloat(goalCurrent || '0'), color: GOAL_COLORS[savingsGoals.length % GOAL_COLORS.length] }; setSavingsGoals([...savingsGoals, newGoal]); setIsGoalModalOpen(false); }} className="space-y-4">
              <input type="text" placeholder="Title" value={goalTitle} onChange={(e) => setGoalTitle(e.target.value)} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} required />
              <input type="number" placeholder="Target" value={goalTarget} onChange={(e) => setGoalTarget(e.target.value)} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} required />
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold">Create Goal</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className={`w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 transition-all ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); if(!newCatName) return; const newCat: CategoryDefinition = { id: `cat-${crypto.randomUUID()}`, name: newCatName, iconName: newCatIcon }; setCategories([...categories, newCat]); setIsCategoryModalOpen(false); setNewCatName(''); }} className="space-y-4">
              <input type="text" placeholder="Name" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50'}`} required />
              <div className="grid grid-cols-6 gap-2 p-2 bg-gray-50 dark:bg-slate-800 rounded-2xl max-h-48 overflow-y-auto">
                {ICON_OPTIONS.map(icon => (
                  <button key={icon} type="button" onClick={() => setNewCatIcon(icon)} className={`w-10 h-10 flex items-center justify-center rounded-lg ${newCatIcon === icon ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-slate-700'}`}>
                    {getIcon(icon)}
                  </button>
                ))}
              </div>
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold">Add Category</button>
            </form>
          </div>
        </div>
      )}
      ...
      {(transactionToDelete || goalToDelete || categoryToDelete) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
          <div className={`w-full max-w-xs p-6 rounded-3xl ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
            <h3 className="text-lg font-bold text-center mb-2">Delete this item?</h3>
            <p className="text-sm text-center text-gray-500 mb-6">This action is permanent.</p>
            <div className="flex flex-col gap-2">
              <button onClick={() => { 
                if (transactionToDelete) setTransactions(transactions.filter(t => t.id !== transactionToDelete)); 
                if (goalToDelete) setSavingsGoals(savingsGoals.filter(g => g.id !== goalToDelete)); 
                if (categoryToDelete) { setCategories(categories.filter(c => c.id !== categoryToDelete)); setTransactions(prev => prev.map(t => t.category === categoryToDelete ? { ...t, category: 'cat-other' } : t)); } 
                setTransactionToDelete(null); setGoalToDelete(null); setCategoryToDelete(null); 
              }} className="w-full py-3 bg-rose-600 text-white rounded-xl font-bold">Delete</button>
              <button onClick={() => { setTransactionToDelete(null); setGoalToDelete(null); setCategoryToDelete(null); }} className="w-full py-3 bg-gray-100 dark:bg-slate-800 rounded-xl font-semibold">Cancel</button>
            </div>
          </div>
        </div>
      )}
